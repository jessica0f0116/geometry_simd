 ...
 */
inline Polyline generate_random_line(size_t num_points, double scale = 100.0, unsigned seed = 42) {
 ...
 */
inline Polyline generate_sine_wave(size_t num_points, double amplitude = 10.0, double frequency = 1.0) {
    Polyline line;
    line.reserve(num_points);
 ...
 */
inline Polyline generate_noisy_line(size_t num_points, double noise_level = 1.0, unsigned seed = 42) {
    std::mt19937 rng(seed);
 ...
 */
inline Polyline generate_coastline(size_t num_points, unsigned seed = 42) {
    std::mt19937 rng(seed);
